...
if __name__ == "__main__":
    ...
    arms_start = datetime(2020, 1, 28, 11, 0, 0)
    arms_end = datetime(2020, 1, 28, 12, 0, 0)
    ...
    for t_idx in range(len(arms_time)):
        ...
        Psv = np.zeros((len(Depth), len(freqs)), dtype=np.complex64)

        for f_idx, freq, result, dt in results:
            Range, Depth, P = rampe_output_reader(t_curr, freq)
            range_idx = np.argmin(np.abs(Range - receiver_range))
            Psv[:, f_idx] = P[:, range_idx]
        ...
        # Remove RAMPE input/output files to save space
        ...
        clean_dir = "rm -rf " + ram_input_dir
        subprocess.run(clean_dir, shell=True)

    # Save frequency/time file
    freq_time_file = os.path.join(save_file_dir, "rampe_freq_time_28_12.mat")
    hdf5storage.savemat(
        freq_time_file,
        {
            "freqs": freqs,
